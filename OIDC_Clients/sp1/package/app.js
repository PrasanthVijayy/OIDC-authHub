...
const idp_url = new URL(process.env.IDP_URL || "http://localhost:4000"); // IDP URL
...
console.log("SP I - CORS", corsOptions);

// Session middleware
app.use(
  session({
    name: "clientSession-1",
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!oidcClient.serverMetadata().supportsPKCE()) {
      console.warn("PKCE is not supported, using NONCE!");
...
app.get("/callback", setNoCache, async (req, res) => {
  if (!oidcClient) {
    return res.status(500).send("OIDC client not configured.");
  }
  ...
  try {
    ...
    req.session.destroy(async (err) => {
      ...
      try {
        ...
        res
          .status(200)
          .send({ message: "Logout successful, but no redirect." });
      } catch (error) {
...
app.post("/backchannel-logout", setNoCache, async (req, res) => {
  try {
    ...
    if (!logoutToken) {
      console.error(
        "Invalid backchannel logout request - missing logout_token."
      );
...
async function handleLogoutTokenVerification(logoutToken, req, res) {
  try {
    ...
    req.sessionStore.all((err, sessions) => {
      ...
      // Find session where incoming sub is present in our session to find and delete
...
//   console.log(`Server running with machine IP: ${process.env.SP_URL}:${port}`);
// });

/* ---------- SERVER SETUP WITHOUT SSL ---------- */
app.listen(port, "0.0.0.0", () => {
  //  console.log(`Client is running onhttp://localhost:${port}`);
